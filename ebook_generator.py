...
class EbookGenerator:
    ...
    def create_ebook_markdown(self, niche):
        ...
        for i, chapter in enumerate(details['chapters'], 1):
            print(f"  Generating chapter {i}/{len(details['chapters'])}: {chapter}")
    ...
    def generate_all(self):
        ...
        print(f"Total niches: {len(self.niches)}")
        print("=" * 50)
        
        for niche in self.niches.keys():
            print(f"\n📚 Generating: {self.niches[niche]['title']}")
            print(f"   Chapters: {len(self.niches[niche]['chapters'])}")
            print(f"   Target: {self.niches[niche]['target']}")
            
            content = self.create_ebook_markdown(niche)
            self.save_ebook(niche, content)
            
            print(f"   ✅ Completed: {self.niches[niche]['title']}")
            print("   ⏳ Waiting before next ebook...")
            time.sleep(5)
        
        print("\n" + "=" * 50)
        print("🎉 ALL EBOOKS GENERATED SUCCESSFULLY!")
        print(f"Total files: {len(self.generated_files)}")
    ...
    def create_summary(self):
        ...
        with open("ebook_summary.md", 'w', encoding='utf-8') as f:
            f.write(summary)
        
        print("📊 Summary created: ebook_summary.md")
...
if __name__ == "__main__":
    ...
    if choice == "1":
        ...
        for niche in generator.niches.keys():
            print(f"\nTesting: {generator.niches[niche]['title']}")
